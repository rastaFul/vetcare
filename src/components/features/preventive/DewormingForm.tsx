...
import { ApplyDewormingSchema, COMMON_DEWORMINGS } from '@/modules/preventive/application/dtos/PreventiveDTO'
import { z } from 'zod'

type FormInput = z.input<typeof ApplyDewormingSchema>
type FormOutput = z.output<typeof ApplyDewormingSchema>
...
export function DewormingForm({ animalId, onSubmit, onCancel }: Props) {
  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
  } = useForm<FormInput, unknown, FormOutput>({
    resolver: zodResolver(ApplyDewormingSchema),
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium mb-1">Vermífugo</label>
        <input
          list="dewormings-list"
          {...register('medication')}
          className="w-full border rounded px-3 py-2"
          placeholder="Ex: Drontal Plus..."
        />
        <datalist id="dewormings-list">
          {COMMON_DEWORMINGS.map((v) => <option key={v} value={v} />)}
        </datalist>
