...
describe('CompleteSession use case', () => {
  it('session found → completes and updates', async () => {
    const repo = makeRepo()
    const session = makeSession()
    repo.findById.mockResolvedValue(session)
    const useCase = new CompleteSession(repo)
    await useCase.execute({ tenantId: 'tenant-1', sessionId: session.id, notes: 'Feito', priceCharged: 200 })
    expect(session.status).toBe('COMPLETED')
    expect(session.notes).toBe('Feito')
    expect(session.priceCharged).toBe(200)
