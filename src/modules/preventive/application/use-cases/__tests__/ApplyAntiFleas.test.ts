import { ApplyAntiFleas } from '../ApplyAntiFleas'
...
class InMemoryAntiFleasRepository implements IAntiFleasRepository {
  records: AntiFleasRecord[] = []
  ...
  async findById(id: string): Promise<AntiFleasRecord | null> {
  ...
  async findUpcoming(tenantId: string, daysAhead: number): Promise<AntiFleasRecord[]> {
    ...
    return this.records.filter(
      (r) =>
        r.tenantId === tenantId &&
        r.nextApplicationAt &&
        r.nextApplicationAt >= now &&
        r.nextApplicationAt <= future
    )
  }
}
...
describe('ApplyAntiFleas', () => {
  let antiFleasRepo: InMemoryAntiFleasRepository
  let animalRepo: InMemoryAnimalRepository
  let calendarService: MockCalendarService
  let useCase: ApplyAntiFleas

  beforeEach(() => {
    antiFleasRepo = new InMemoryAntiFleasRepository()
    animalRepo = new InMemoryAnimalRepository()
    calendarService = new MockCalendarService()
    useCase = new ApplyAntiFleas(antiFleasRepo, animalRepo, calendarService)
  })

  it('should create anti-fleas record for valid animal', async () => {
    ...
    expect(result.medication).toBe('Frontline')
    expect(result.animalId).toBe('animal-1')
    expect(result.tenantId).toBe('tenant-1')
    expect(antiFleasRepo.records).toHaveLength(1)
  })
  ...
  it('should create calendar reminder when requested', async () => {
    animalRepo.addAnimal(makeAnimal())
  ...
  it('should create record even when calendar fails', async () => {
    ...
    expect(result.medication).toBe('Bravecto')
