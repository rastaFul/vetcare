import { ApplyDeworming } from '../ApplyDeworming'
...
class InMemoryDewormingRepository implements IDewormingRepository {
  ...
  async findById(id: string): Promise<DewormingRecord | null> {
...
describe('ApplyDeworming', () => {
  let dewormingRepo: InMemoryDewormingRepository
  let animalRepo: InMemoryAnimalRepository
  let calendarService: MockCalendarService
  let useCase: ApplyDeworming

  beforeEach(() => {
    dewormingRepo = new InMemoryDewormingRepository()
    animalRepo = new InMemoryAnimalRepository()
    calendarService = new MockCalendarService()
    useCase = new ApplyDeworming(dewormingRepo, animalRepo, calendarService)
  })

  it('should create deworming record for valid animal', async () => {
    ...
    expect(result.medication).toBe('Drontal Plus')
    expect(result.animalId).toBe('animal-1')
    expect(result.tenantId).toBe('tenant-1')
    expect(dewormingRepo.records).toHaveLength(1)
  })
  ...
  it('should create record even when calendar fails', async () => {
    ...
    expect(result.medication).toBe('Milbemax')
